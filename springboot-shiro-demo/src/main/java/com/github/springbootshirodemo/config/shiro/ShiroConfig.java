...
import com.github.springbootshirodemo.config.shiro.cache.CustomCacheManager;
import com.github.springbootshirodemo.config.shiro.jwt.JwtFilter;
...
public class ShiroConfig {
    ...
    /**
     * 配置使用自定义Realm，关闭Shiro自带的session,Disabling Subject State Session Storage
    ...
    public DefaultWebSecurityManager defaultWebSecurityManager(UserRealm userRealm) {
        ...
        // 使用自定义Realm,注意必须开启缓存，否则redis缓存不生效
        userRealm.setCachingEnabled(true);
        userRealm.setAuthenticationCachingEnabled(true);
        userRealm.setAuthorizationCachingEnabled(true);
        // 需要指定token的类型，shiro默认是UsernamePasswordToken
        userRealm.setAuthenticationTokenClass(BearerToken.class);
        defaultWebSecurityManager.setRealm(userRealm);
        // 关闭Shiro自带的session
        DefaultSubjectDAO subjectDAO = new DefaultSubjectDAO();
        DefaultSessionStorageEvaluator defaultSessionStorageEvaluator = new DefaultSessionStorageEvaluator();
        // Disabling Subject State Session Storage,设置为false关闭
